...
function addMissingNodes(fromNode: any, toNode: any): boolean {
    ...
    if (fromNode instanceof YAMLMap && toNode instanceof YAMLMap) {
        for (const pair of fromNode.items) {
            ...
            hasChanges = hasChanges || addMissingNodes(fromVal, toVal);
        }
        return hasChanges;
    }
...
function removeMissingNodes(fromNode: any, toNode: any): boolean {
    ...
    if (fromNode instanceof YAMLMap && toNode instanceof YAMLMap) {
        for (let i = toNode.items.length - 1; i >= 0; i--) {
            ...
            hasChanges = hasChanges || removeMissingNodes(fromVal, toPair.value);
        }
        return hasChanges;
    }
    ...
    if (fromNode instanceof YAMLMap && toNode instanceof YAMLMap) {
        for (const fromPair of fromNode.items) {
            ...
            if (
                ...
                hasChanges = hasChanges || mergeOverwriteNodes(fromVal, toVal);
            } else {
