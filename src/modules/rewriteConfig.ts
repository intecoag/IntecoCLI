...
async function configRewrite(_cli: unknown): Promise<void> {
    console.log()
    ...
    if (success) {
        ...
        const editorJwegasProperties = createEditor(homedir + path.sep+"jwegas.properties") as PropertiesEditor;
        editorJwegasProperties.set("user.mandant", responses.mnr.toString());
        editorJwegasProperties.save();

        // Rewrite wegas.properties
        const editorWegasProperties = createEditor(config.configIndividualPath + path.sep+"wegas.properties") as PropertiesEditor;
