...
async function githubSecurityAdvisories() {
  ...
  try {
    ...
    if (message.startsWith('401')) {
      ...
      console.error(chalk.red(`x ${message} - Organization or repository not found`));
    } else {
...
async function fetchSecurityAdvisories(organization: string, repository: string, token: string): Promise<SecurityAlert[]> {
  const url = `https://api.github.com/repos/${organization}/${repository}/dependabot/alerts`;

  try {
    const res = await fetchGithubAPI(url, token);
    const alerts = await res.json() as SecurityAlert[];
