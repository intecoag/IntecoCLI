...
type SyncDirection =
    ...
    | "import_all"
    | "export_all"
    | "import_all_individuals"
    | "export_all_individuals"
    | "sync_to_configIndividual";

type SyncType = "UPDATE" | "OVERWRITE" | "CREATE_IF_NOT_EXISTS";

type SyncResponses = {
    direction: SyncDirection;
    configNameSource?: string;
    configNameTarget?: string;
    configIndividualSelection?: string;
    dryRun: boolean;
    type: SyncType;
};
...
function processMultiple(responses: SyncResponses, dryRun: boolean, sourcePaths: string[], destPaths: string[]) {
    console.log();

    switch (responses.type) {
        case 'UPDATE':
            console.log(chalk.yellow(`Updating files from ${sourcePaths.join(', ')} -> ${destPaths.join(', ')}`));
            const updateSummary = { added: 0, updated: 0 };
            for(let i = 0; i < sourcePaths.length && i < destPaths.length; i++) {
                FS.copyUpdatedFiles(sourcePaths[i], destPaths[i], dryRun, updateSummary, ["wegas.properties", "path.yaml"]);
            }
            console.log();
            console.log(chalk.green(`Summary: ${updateSummary.updated} files added or updated.`));
            break;

        case 'OVERWRITE':
            console.log(chalk.red(`Overwriting files from ${sourcePaths.join(', ')} -> ${destPaths.join(', ')}`));

            const overwriteSummary = { copied: 0 };
            let deletedCount = 0;
            for(let i = 0; i < sourcePaths.length && i < destPaths.length; i++) {
                ...
                FS.copyAllFiles(sourcePaths[i], destPaths[i], dryRun, overwriteSummary, ["wegas.properties", "path.yaml"]);
            }

            console.log();
            console.log(chalk.green(`Summary: Deleted ${deletedCount} items, Copied ${overwriteSummary.copied} files.`));
            break;
    }
    ...
    if (existsSync(dirPath)) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                shouldKeepDirectory = shouldKeepDirectory || deletedCount != 0;
            } else {
                deletedCount++;
                if(dryRun) {
