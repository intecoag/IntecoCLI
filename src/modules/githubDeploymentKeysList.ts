 ...
 */
async function fetchDeploymentKeys(baseUrl: string, token: string): Promise<DeploymentKey[]> {
  try {
    return await fetchPaginatedGithubAPI<DeploymentKey>(baseUrl, token);
  } catch {
...
export async function listGithubDeploymentKeys() {
  ...
  try {
    ...
    const repos = await fetchPaginatedGithubAPI<RepoInfo>(baseReposUrl, token);
