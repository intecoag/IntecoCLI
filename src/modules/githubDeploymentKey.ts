...
import { getGithubToken, fetchPaginatedGithubAPI } from '../utils/github/github.js';
...
export async function addGithubDeploymentKey() {
  ...
  const orgResponse = await prompts([
    {
      ...
      initial: 'intecoag',
      validate: (value: string) => value.length > 0 || 'Organization name is required'
    }
  ], {
    onCancel: () => {
      console.log();
  ...
  try {
    ...
    const repos = await fetchPaginatedGithubAPI<RepoInfo>(baseUrl, token);
    ...
    const publicKeyResponse = await prompts([
      ...
      },
      {
        ...
        validate: (value: string) => value.length > 0 || 'Key name is required'
      }
    ], {
      onCancel: () => {
        console.log();
    ...
    if (message.startsWith('401')) {
      ...
      console.error(chalk.red(`x ${message} - Organization not found`));
    } else {
...
function displayDeploymentKeyScript(organization: string, repository: string) {
...
function generateBashScript(keyPath: string, keyName: string, organization: string, repository: string): string {
...
async function createDeploymentKey(organization: string, repository: string, publicKey: string, keyName: string): Promise<void> {
  ...
  try {
    ...
    if (message.includes('not found')) {
      console.error(chalk.red(`x Repository not found: ${organization}/${repository}`));
    } else if (message.includes('already exists')) {
      console.error(chalk.red(`x A key with this name already exists`));
    } else if (message.includes('Permission denied')) {
      console.error(chalk.red(`x Permission denied. You may not have write access to this repository`));
    } else {
...
function displayCloneCommand(organization: string, repository: string) {
  ...
  console.log(chalk.green(`ok SSH key generated and stored locally`));
  console.log(chalk.green(`ok Deployment key added to ${organization}/${repository}`));
  console.log(chalk.green(`ok SSH config configured for easy access`));
  console.log();
}
