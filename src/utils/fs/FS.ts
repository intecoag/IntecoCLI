...
type PickerChoice = { title: string; value: string; isDirectory: boolean };

export class FS {
    static copyUpdatedFiles(sourceDir: string, destDir: string, dryRun = false, stats: CopyUpdatedStats = { added: 0, updated: 0 }, filenameBlacklist: string[] = [], onlyCopyNonExistant = false): void {
    ...
    static copyAllFiles(sourceDir: string, destDir: string, dryRun = false, stats: CopyAllStats = { copied: 0 }, filenameBlacklist: string[] = []): void {
        if (!existsSync(destDir)) {
            if (dryRun) {
                console.log(chalk.gray(`[DryRun] Would create directory: ${destDir}`));
            } else {
    ...
    static async filePicker(startDir = process.cwd(), navRootDir = process.cwd()): Promise<string[]> {
